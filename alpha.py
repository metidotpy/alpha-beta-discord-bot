...
def insert_happy(con,cur):
    command="INSERT INTO Feeling VALUES('happy',0)"
    cur.execute(command)
    con.commit()

#update Feels
def update_happy(con,cur):
    command="UPDATE Feeling SET Grade=Grade+1 WHERE Feels='happy';"
    cur.execute(command)
    con.commit()
#fetch happy
def get_happy(con,cur):
    cur.execute("SELECT Grade FROM Feeling WHERE Feels = 'happy';")
    rows=cur.fetchone()
    for row in rows:
        return row
#clear function
def clear_data(con,cur):
    command="UPDATE Feeling SET Grade=0 WHERE Feels = 'happy';"
    cur.execute(command)
    con.commit()
#insert notes
...
client=commands.Bot(command_prefix="beta ",help_command=None)


#this is on_ready function
@client.event
async def on_ready():
    await client.change_presence(activity=discord.Activity(type=discord.ActivityType.listening,name="beta help"))
    print("WE ARE READY NOW")

answers=["**OK, It's So Nice**","**OK, I Know Life Is Beautiful**","**OK, Making It To Better**", "**You Are a Winner**"]
...
async def help(ctx):
    ...
    embed.add_field(name="Add Your Happy Feels To DataBase",value="**`feels *`**",inline=False)
    embed.add_field(name="Show Your Happy Feels",value="**`time`**, **`much`**",inline=False)
...
async def _time(ctx):
    if ctx.author.id == config("USER_ID"):
        msg=await ctx.send("You Are Feeling Happy **{}** Times".format(get_happy(con,cur)))
